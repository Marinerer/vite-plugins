...
describe('createRewrites function', () => {
  ...
  it('should generate basic rewrites for pages', () => {
    ...
    // Expected number of rules: 3 pages * 1 rule each + 1 for /user/profile/index + 1 for __*__ + 1 for rewriteWhitelist (if any) + 1 for root
  ...
  it('should handle rewriteWhitelist option', () => {
    const viteConfig = mockViteConfig('/');
    const options: PluginOptions = {
      rewriteWhitelist: /^\/custom-whitelist\//,
    };
